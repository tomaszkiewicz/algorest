...
var trimInputObjectToTableColumns = function(obj, cols) {
  var cols = cols.map(function(c) { return changeCase.camelCase(c); });
...
function getColumns(req, tableName, options, ignoreOData) {
  if(!schema[tableName])
    schemaSynchronizer.synchronizeTableSchema(tableName, schema);

  var outputCols = schema.tables[tableName].columns.map(function (col) { return col.name });

  if(options.oDataSelect && !ignoreOData) {
    var selectCols = utils.tryEval(options.oDataSelect, req);
