var sql = require('sql-bricks');
var security = require('./security');
var helpers = require('./helpers');
var utils = require('./utils');

function buildBrowseQuery(req, tableName, options) {
  var cols = security.getColumns(req, tableName, options);
  var colsNoOData = security.getColumns(req, tableName, options, true);
  var colsAs = helpers.getAliasedColumnList(cols);

  var query = (sql
    .select(colsAs)
    .from(tableName)
  );

  applyWhere(req, options, query);

  var oDataWhere = utils.tryEval(options.oDataWhere, req, colsNoOData) || null;

  if(oDataWhere)
    query = query.where(oDataWhere);

  var top = utils.tryEval(options.oDataTop, req) || null;

  if(top)
    query = query.limit(top);

  var skip = utils.tryEval(options.oDataSkip, req) || null;

  if(skip)
    query = query.offset(skip);

  var orderBy = utils.tryEval(options.oDataOrderBy, req) || null;

  if(orderBy) {
    orderBy = helpers.renamePropertiesToSnakeCase(orderBy);

    var orderCols = [];

    for(var i = 0, item; item = orderBy[i++];)
      for(var prop in item)
        orderCols.push(prop + ' ' + item[prop]);

    query = query.orderBy(orderCols.join(', '));
  }

  return query;
}

function buildReadQuery(req, tableName, options, id) {
  var cols = security.getColumns(req, tableName, options);
  var colsAs = helpers.getAliasedColumnList(cols);

  var query = (sql
    .select(colsAs)
    .from(tableName)
    .where({ id: id })
  );

  applyWhere(req, options, query);

  return query;
}

function buildEditQuery(req, tableName, options, newItem, id) {
  var cols = security.getColumns(req, tableName, options);
  var colsAs = helpers.getAliasedColumnList(cols);

  var query = (sql
    .update(tableName,
      helpers.formatUpdateArrayFields(
        helpers.renamePropertiesToSnakeCase(newItem)
      )
    )
    .where({ id: id })
    .returning(colsAs)
  );

  applyWhere(req, options, query);

  return query;
}

function buildAddQuery(req, tableName, options, newItem) {
  var cols = security.getColumns(req, tableName, options);
  var colsAs = helpers.getAliasedColumnList(cols);

  // TODO change do be:
  /*INSERT INTO accounts select * from (
   (select * from accounts limit 0)
   UNION ALL
   values(-1, 'abc', 'pln', 2, 3, 4, '123456', 'abcvdef')
   ) x
   where user_id=7*/

  var query = (sql
    .insert(tableName)
    .values(helpers.renamePropertiesToSnakeCase(newItem))
    .returning(colsAs)
  );

  return query;
}

function buildDeleteQuery(req, tableName, options, id) {
    var cols = security.getColumns(req, tableName, options);
    var query = null;

    if(options.softDeleteColumn) {
        if(cols.indexOf(options.softDeleteColumn) != -1) {
            var tmp = {};
            tmp[options.softDeleteColumn] = true;
            query = sql.update(tableName, tmp).where({ id: id });
        }
        else
        {
            throw new Error('softDeleteColumn ' + options.softDeleteColumn + ' does not exists.');
        }
    } else {
        query = (sql
            .delete()
            .from(tableName)
            .where({ id: id })
            );
    }

    applyWhere(req, options, query);

    return query;
}

function applyWhere(req, options, query) {
  var where = utils.tryEval(options.where, req, sql);

  if(where)
    query = query.where(helpers.renamePropertiesToSnakeCase(where));

  if(options.softDeleteColumn) {
    var allowOperations = utils.tryEval(options.allowOperationsOnDeleted, req);

    if(!allowOperations) {
      var tmp = {};
      tmp[options.softDeleteColumn] = false;

      query = query.where(tmp);
    }
  }
}

module.exports = exports = {
  buildBrowseQuery: buildBrowseQuery,
  buildReadQuery: buildReadQuery,
  buildEditQuery: buildEditQuery,
  buildAddQuery: buildAddQuery,
  buildDeleteQuery: buildDeleteQuery
};