...
var rest = function (tableName, options) {
  ...
  var browseAction = function (req, res) {
    ...
    try {
      ...
      var result = executor.execute(query).rows;

      if(options.beforeSendCollection){
        try {
          if(options.beforeSendCollection instanceof Array)
            for(var i = 0; i < options.beforeSendCollection.length; i++)
              await(options.beforeSendCollection[i](result, defer()));
          else
            await(options.beforeSendCollection(result, defer()));
        } catch(ex) {
          console.log(ex);
        }
      }
  ...
  var readAction = function (req, res) {
    ...
    try {
      ...
      if(options.beforeSend){
        try {
          if(options.beforeSend instanceof Array)
            for(var i = 0; i < options.beforeSend.length; i++)
              await(options.beforeSend[i](obj, defer()));
          else
            await(options.beforeSend(obj, defer()));
        } catch(ex) {
          console.log(ex);
        }
      }
